...
/// Strategy interface for rendering captured ink strokes into embeddable assets.
abstract class SignatureRenderer {
  const SignatureRenderer();

  /// Renders the provided signature strokes into an image payload.
  Future<Uint8List> call({required PdfSignatureData signature, double? targetHeight});
}

/// Default renderer that paints strokes onto an in-memory canvas and exports PNG bytes.
class PaintingSignatureRenderer extends SignatureRenderer {
  const PaintingSignatureRenderer();

  @override
  Future<Uint8List> call({required PdfSignatureData signature, double? targetHeight}) async {
